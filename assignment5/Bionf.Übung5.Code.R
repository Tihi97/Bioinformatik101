...
pkgs <- c(
  "rtracklayer",
  "TFBSTools",
  "JASPAR2014", "BSgenome.Hsapiens.UCSC.hg18", "TxDb.Hsapiens.UCSC.hg18.knownGene" )
...
score(ER) <- as.numeric(ER$name) # overwrite the name column
ER$name <- paste("ER_peaks", 1:length(ER), sep="_")
# use the names() function
names(ER) <- ER$name ER
print(ER)

bp <- barplot(c(length(ER), length(FOXA1)), names=c("ER", "FOXA1")) # add actual values as text lables to the plot
...
gene.ids <- names(ER.genes) # take only unique ids
